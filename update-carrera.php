<?php
	...
	if(isset($_POST['guardar'])){
		...
		if(!empty($nombre) && !empty($correo) && !empty($contraseña))
		{
			...
			{
				$consulta_update=$conn->prepare(' UPDATE clientes SET  
					nombre=:nombre,
					correo=:email,
					contraseña=:password,
					WHERE idUser=:idUser;'
	...
	<div class="contenedor">
		...
		<form action="" method="post">
			<div class="form-group">
				<input type="text" name="email" value="<?php if($resultado) echo $resultado['Correo electronico']; ?>" class="input__text">
				<input type="text" name="name" value="<?php if($resultado) echo $resultado['Nombre']; ?>" class="input__text">
			</div>
			<div class="form-group">
				<input type="text" name="password" value="<?php if($resultado) echo $resultado['Contraseña']; ?>" class="input__text">
			</div>
